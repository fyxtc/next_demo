...
def demo(cid=1, page=0):
    ...
    if cid == 15: # 全部
        sql = "select * from demo limit ? offset ?"
        params = params[1:]
...
def info(name='棋灵王'):
    ...
    # res = query_db("select * from demo where mid = ?", [mid], True)
    # 改为传name了。。。为了seo
    print("info name " + name)
    res = query_db("select * from demo where name = ?", [name], True)
    print(res)
    return jsonify(res)
...
def read_chapter(mid, chapter=1):
    ...
    chapter_images = query_db("select image_base_url, all_chapters_pages, chapter_start_index, all_vols_pages, name from demo where mid = ?", [mid], True)
...
def search(key):
    if key:
        sql = "select * from demo where name like '%{0}%' or author like '%{1}%' ".format(key, key) 
...
def start_requests():
    ...
    if not exist:
        # print(os.system("pwd"))
        os.system('sqlite3 ../server/demo.db ".read ../server/demo.sql"')
